...
app= Flask(__Weapons__)

#Connect to SQLite
def DBConnection():
    return sqlite3.connect('Weapons.db')

#Request value of all swords
def GetSwordsFromDB():
    with closing(DBConnection()) as connection, connection.cursor() as cursor:
        cursor.execute('SELECT WeaponName, WeaponRarity, Descritpion, Picture FROM swords')
...
def GetSwordFromDB(WeaponName)
    with closing(DBConnection()) as connection, connection.cursor() as cursor:
        cursor.execute('SELECT WeaponName, WeaponRarity, Description, Picture FROM swords WHERE name= ?', (WeaponName))
